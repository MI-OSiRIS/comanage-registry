...
<table id="co_service_token_settings" class="ui-widget">
  <thead>
    <tr class="ui-widget-header">
      ...
      <th><?php print $this->Paginator->sort('CoServiceTokenSetting.co_ceph_provisioner_target_id', _txt('pl.coservicetoken.token.ceph_provisioner'));?></th>
      <th><?php print $this->Paginator->sort('CoServiceTokenSetting.co_ldap_provisioner_target_id', _txt('pl.coservicetoken.token.ldap_provisioner'));?></th>
    </tr>
  </thead>
  ...
  <tbody>
    ...
    <tr class="line<?php print ($i % 2)+1; ?>">
      ...
      <td>
        <?php
          // ceph provisioner_id select for keys from ceph
        
          $attrs = array();
          $attrs['empty'] = true;
          $attrs['value'] = (isset($c['CoServiceTokenSetting']['co_ceph_provisioner_target_id'])
                             ? $c['CoServiceTokenSetting']['co_ceph_provisioner_target_id']
                             : '');
          
          print $this->Form->select('CoServiceTokenSetting.' . $i . '.co_ceph_provisioner_target_id',
                                    $vv_ceph_provisioners,
                                    $attrs);

          if($this->Form->isFieldError('CoServiceTokenSetting.' . $i . '.co_ceph_provisioner_target_id')) {
            print $this->Form->error('CoServiceTokenSetting.' . $i . '.co_ceph_provisioner_target_id');
          }
       ...
       <td>
        <?php
          // ldap provisioner_id select to lookup ldap info on gid needed in ceph key caps
          $attrs = array();
          $attrs['empty'] = true;
          $attrs['value'] = (isset($c['CoServiceTokenSetting']['co_ldap_provisioner_target_id'])
                             ? $c['CoServiceTokenSetting']['co_ldap_provisioner_target_id']
                             : '');
          
          print $this->Form->select('CoServiceTokenSetting.' . $i . '.co_ldap_provisioner_target_id',
                                    $vv_ldap_provisioners,
                                    $attrs);

          if($this->Form->isFieldError('CoServiceTokenSetting.' . $i . '.co_ldap_provisioner_target_id')) {
            print $this->Form->error('CoServiceTokenSetting.' . $i . '.co_ldap_provisioner_target_id');
          }
